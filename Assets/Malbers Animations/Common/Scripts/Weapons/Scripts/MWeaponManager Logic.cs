    ...
    public partial class MWeaponManager
    {
        ...
        public virtual void Equip_Weapon()
        {
            ...
            Weapon.PlaySound(WSound.Equip);                             //Play Equip Sound
                                                                        // WeaponIKWeight = Weapon.TwoHandIK ? 1 : 0;
            ...
            if (UseHolsters)                                                             //If Use holster Means that the weapons are on the holster
            {
                ...
                var PosOffset = Weapon.PositionOffset;
                var RotOffset = Weapon.RotationOffset;
                var ScalOffset = Weapon.ScaleOffset;

                if (IgnoreHandOffset.Value)
                {
                    PosOffset = Vector3.zero;
                    RotOffset = Vector3.zero;
                    ScalOffset = Vector3.one;
                }
                ...
                if (SmoothEquip)
                {
                    ...
                    StartCoroutine(
                        MTools.AlignTransformLocal(Weapon.transform, PosOffset, RotOffset, ScalOffset, HolsterTime));
                }
                else
                {
                    //Local position when is Parent to the weapon
                    Weapon.transform.SetLocalTransform(PosOffset, RotOffset, ScalOffset);
                }
            ...
            {
                if (!Weapon.gameObject.IsPrefab()) //Check that the weapon is not a prefab
                {
                    ParentWeapon();
                    if (IgnoreHandOffset.Value)
        ...
        /// <summary> If false the Smooth equipment will fail </summary>
        private bool SmoothEquip;
