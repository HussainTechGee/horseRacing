...
namespace MalbersAnimations.Utilities
{
    ...
    public class Aim : MonoBehaviour, IAim, IAnimatorListener
    {
        ...
        public void CalculateAngles(float time)
        {
            ...
            //else
            //{
            //    HorizontalAngle = Mathf.Lerp(HorizontalAngle, HorizontalAngle_Raw, AngleLerp * time);
            //    VerticalAngle = Mathf.Lerp(VerticalAngle, VerticalAngle_Raw, AngleLerp * time);
            //}
        ...
        public RaycastHit DirectionFromCamera(bool useray)
        {
            ...
            if (ScreenCenter != Vector3.zero) CamRay = cam.ScreenPointToRay(ScreenCenter);
            else CamRay = new Ray(cam.transform.position, cam.transform.forward);

        

            CamRay.origin += cam.transform.forward * ForwardCam; //Push the ray forward so it does not touch near colliders

            var MaxPoint =  CamRay.GetPoint(AimHit.distance);
            ...
            if (useray)
            {
                //Debug.DrawLine(CamRay.origin, MaxPoint, Color.red);
    ...
    public class AimEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (Application.isPlaying)
            {
                EditorGUILayout.BeginVertical(m.AimTarget != null ? MTools.StyleBlue : MTools.StyleGreen);
